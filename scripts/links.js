  ...
  for (const file of files) {
    ...
    for (const link of links) {
      ...
      if (broken) {
        ...
        styledConsole(
          `⚠️  Broken link: "${broken.linkText}" (${broken.linkUrl}) in ${relativeFilePath} on line ${broken.lineNumber}`,
          'warning'
        )
...
function displayHelp() {
  ...
  console.log('\nExamples:')
  console.log(
    '  node scripts/links.js                   # Check for broken links (exits with error if found)'
  )
  console.log(
    '  node scripts/links.js --force           # Check for broken links but continue regardless'
  )
...
function promptToContinue() {
  return new Promise((resolve) => {
    process.stdout.write(
      '\x1b[33m⚠️  Broken links found. Do you want to continue anyway? (y/N): \x1b[0m'
    )
...
async function main() {
  try {
    ...
    if (brokenCount > 0) {
      ...
      if (forceFlag) {
        styledConsole(
          '⚠️  PROCEEDING WITH BUILD DESPITE BROKEN LINKS (--force flag used)',
          'warning'
        )
        printDivider()
      ...
      if (promptFlag) {
        ...
        if (shouldContinue) {
          styledConsole(
            '⚠️  PROCEEDING WITH BUILD DESPITE BROKEN LINKS (user confirmed)',
            'warning'
          )
          printDivider()
          process.exit(0) // Exit with success code to allow build to continue
