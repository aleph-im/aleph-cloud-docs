...
async function encryptAndStore(data, recipientPublicKey) {
  ...
  const sharedSecret = ethers.utils.keccak256(
    ethers.utils.concat([ethers.utils.arrayify(await ephemeralWallet.signMessage(publicKeyBytes)), publicKeyBytes])
  )
...
documents.posts.forEach((doc) => {
  if (doc.content.metadata && doc.content.metadata.language === 'en' && doc.content.metadata.version === '1.0') {
